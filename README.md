...
전체 304페이지. 색인(index) 299페이지(읍면동 본문 2,000~2,500자 확장 후 색인).
시→구→동 클릭 동선 완결: 시 페이지 → 일반구 → 일반구 대표 행정동 전부 클릭 연결.
...
- 읍면동 본문을 **2,000~2,500자**(구글 가이드라인)로 확장하여 색인 — 위치·생활권,
  교통·이동, 방문 장소(목적별), 이용 기준, 연결 안내, FAQ, E-E-A-T 바이라인 포함
- **E-E-A-T 신호**: 전 지역 페이지에 작성·검수 바이라인(운영팀·연락처·회사/개인정보 링크)
- 색인: 메인1 + 권역5 + 시군31 + 일반구20 + 생활권25 + 역세권55 + 읍면동156 + 목적별6 = **299페이지**
